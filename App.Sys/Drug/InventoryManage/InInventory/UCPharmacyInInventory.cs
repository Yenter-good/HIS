...
namespace App_Sys.Drug.InventoryManage
{
    internal partial class UCPharmacyInInventory : UserControl, IUCInit
    {
        ...
        private void InitReceipt()
        {
            ...
            var receiptEntities = _pharmacyService.GetPharmacyInOutReceipt(ViewData.Dept.Id, PharmacyInOutReceiptType.入库, this.dtStartDate.Value, this.dtEndDate.Value, this.tbxReceiptCode.Text);
        ...
        private GridRow BuildNewRow(PharmacyInOutInventoryEntity receiptEntity)
        {
            ...
            newRow.Cells[colSourceDept.ColumnIndex].Value = receiptEntity.SourceDept.Name;
        ...
        {
            if (!App.Instance.User.HasPermission(PermissionsContants.Instance.Drug_PharmacyInInventoryReceiptAudit))
            {
            ...
            var result = _pharmacyService.AuditInReceipt(receiptEntity.Id, receiptEntity.SourceDept.Id, receiptEntity.TargetDept.Id);
            if (result.Success)
            {
        ...
        private bool CancelAuditReceipt(PharmacyInOutInventoryEntity receiptEntity)
        {
            ...
            var result = _pharmacyService.CancelAuditInReceipt(receiptEntity.Id, receiptEntity.SourceDept.Id, receiptEntity.TargetDept.Id);
            if (result.Success)
            {
        ...
        {
            FormAddPharmacyInInventoryReceipt form = App.Instance.CreateView<FormAddPharmacyInInventoryReceipt>();
