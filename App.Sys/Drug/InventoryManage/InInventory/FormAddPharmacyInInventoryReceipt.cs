...
{
    internal partial class FormAddPharmacyInInventoryReceipt : BaseForm
    {
        ...
        public FormAddPharmacyInInventoryReceipt(IPharmacyInOutInventoryService pharmacyReceiptService, IDeptService deptService, IInvoiceService invoiceService)
        {
            InitializeComponent();
        ...
        private void InitData()
        {
            ...
            {
                MsgBox.OK("当前科室类型有误,请选择药房科室进行入库");
            ...
            _allDrugEntities = _pharmacyReceiptService.GetAllDrugInfoCanIn(this.fcbSourceDept.SelectedValue.AsLong(0));
        ...
        private void InitUI()
        {
            ...
            {
                var result = _invoiceService.GetInvoiceNumber(InvoiceType.药房申请药品);
                if (result.Success)
                {
        ...
        private void FormAddPharmacyInInventoryReceipt_Shown(object sender, EventArgs e)
        {
            this.ShowMask(() =>
            {
                InitData();
        ...
        private void dgvDrugs_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == this.colQuantity.Index)
            {
                ...
                {
                    sum += row.Cells[colQuantity.Index].Value.AsInt(0) * row.Cells[colPurchasePrice.Index].Value.AsDecimal(0);
                }
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            Receipt.SourceDept = this.fcbSourceDept.SelectedItem as DeptEntity;
            Receipt.TargetDept = Dept;
            ...
            DataResult result;
            if (Operation == DataOperation.New)
                result = _pharmacyReceiptService.AddInInventoryReceipt(Receipt, newDetailEntities);
            else
        ...
        private void fcbSourceDept_TextChanged(object sender, EventArgs e)
        {
            if (MsgBox.YesNo("切换进货科室将会清空待选列表,是否确定") != DialogResult.Yes)
                return;

            this.dgvDrugs.ShowMask(() =>
            {
                _allDrugEntities = _pharmacyReceiptService.GetAllDrugInfoCanIn(this.fcbSourceDept.SelectedValue.AsLong(0));
